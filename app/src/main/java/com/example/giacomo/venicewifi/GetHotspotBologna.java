...
public class GetHotspotBologna extends AsyncTask<Void, Void, Void> {

    Activity mActivity;
    String url = "https://opendata.publicwifi.it/api/v1/layers/wifimetropolitano/nodes.geojson?format=json";

    public GetHotspotBologna(Activity activity){
        super();
        mActivity = activity;
    }
    @Override
...
//            ReadOffilineFile test = new ReadOffilineFile();
        ...
        if  (1==1) {
            ...
            jsonStr = load.loadData("HotspotBologna.txt",mActivity);
        }
        if (jsonStr != null) {
            try {
                ...
                for (int i = 0; i < features.length(); i++) {
                    ...
                    String lon =  c.getString("Cclatiitudine_1553977735");
                    String lat =  c.getString("Cclongitudine_1241027993");

                    // Properties node is JSON Object
                    String codice = c.getString("Cgid_102338");
                    String descrizione = c.getString("Ccdescrizione_1821345524");
