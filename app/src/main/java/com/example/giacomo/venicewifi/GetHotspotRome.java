...
import static com.example.giacomo.venicewifi.ActivityMaps.rome;
...
public class GetHotspotRome extends AsyncTask<Void, Void, Void> {
    ...
    public GetHotspotRome(Activity activity){
        super();
        mActivity = activity;
    }
    @Override
...
//            ReadOffilineFile test = new ReadOffilineFile();
        ...
        if  (1==1) {
            ...
            jsonStr = load.loadData("HotspotRome.txt",mActivity);
        }
        if (rome) {
            try {
                ...
                for (int i = 0; i < features.length(); i++) {
                    ...
                    String codice = properties.getString("name");
                    String descrizione = properties.getString("description");
                    String stato = properties.getString("status");
